...
function draw_barchart1(data_in){
			...
			var svg_barchart = d3.select("#area1").append("svg").transition();
			
			//X Y軸的文字標示，因為是update，所以不能用append，要用Select
			svg_barchart.select(".x axis")
			  .duration(750)
			  .attr("class", "x axis")
		  ...
		  console.log(dataset);
		  var bars = svg_barchart.selectAll(".bar").data(dataset); //格式如果給錯，會出現.selectAll(...).data is not a function
		  
		  
		  bars.transition().duration(300)
			  .attr("x", function(d) { return x2(d.cmonth); })
			  console.log(d.cmonth)
			...
			var rect2 = svg_barchart.selectAll("rect")
				...
				.on("mousemove", function(d) {
					...
					tooltip2.select("text").text(d.cmonth + ' ' + d.cnt);
